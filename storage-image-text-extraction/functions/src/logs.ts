...
export const noContentType = (objectName: string) => {
    logger.log(`File has no Content-Type, no processing is required for '${objectName}}'`);
};
...
export const imageExtractionFailed = (filePath: string) => {
    logger.log(`text extraction did not complete successfully on image ${filePath}`);
}
