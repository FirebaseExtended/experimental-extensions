...
exports.fssentiment = functions.handler.firestore.document.onWrite(async (change) => {
    const { inputFieldName, outputFieldName } = config_1.default;

    if (inputFieldName == outputFieldName) {
        console.log("Sentiment analysis: input field cannot be the same as output field. Please reconfigure your extension.");
        return;
    }
...
const updateSentiment = async (snapshot, sentiment) => {
    ...
    await admin.firestore().runTransaction((transaction) => {
        transaction.update(snapshot.ref, config_1.default.outputFieldName, sentiment);
